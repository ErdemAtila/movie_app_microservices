...
function showRegisterModal() {
    const html = `
        ...
        <div class="form-group">
            ...
            <input type="text" name="address" class="form-input" placeholder="Email or Address">
        </div>
    `;
    
    showModalHtml("Register New Account", html, async (data) => {
        try {
            ...
            if (res.ok) {
                ...
                showToast('Registration failed: ' + (result.message || 'Validation error'), 'error');
            }
        } catch (e) {
            showToast('Error connecting to registration service', 'error');
        }
    }, false); // false for isEdit, meaning button says 'Create'
}
